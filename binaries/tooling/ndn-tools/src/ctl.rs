...
struct Cli {
    ...
    /// NDN identity name to use for signing command Interests
    /// (e.g. `/ndn/router1`).
    ///
    /// Required when the forwarder enforces certificate-based command
    /// authentication (testbed NFD with `rib.localhop_security`). When
    /// omitted, commands are signed with DigestSha256, which is accepted
    /// by ndn-fwd and localhost-configured NFD.
    #[arg(long, value_name = "NDN_NAME")]
    identity: Option<String>,

    /// PIB directory path for `--identity` key lookup
    /// (default: `~/.ndn/pib`).
...
async fn run_nfd(cli: &Cli) -> anyhow::Result<()> {
    ...
    if let Some(identity_str) = &cli.identity {
